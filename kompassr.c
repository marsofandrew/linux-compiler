...
int FDC()                                         /*подпр.обр.пс.опер.DC    */
{
  ...
  {                                              /*то:                     */
    ...
    } else if (!strncmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "BL2", 3)) {
      T_SYM[ITSYM].DLSYM = 2;
      T_SYM[ITSYM].PRPER = 'R';
      if (CHADR % 2) {
        CHADR = (CHADR / 2 + 1) * 2;
  ...
  {                                              /*то:                     */
      ...
      /* следующее:             */
    {
      ...
      PRNMET = 'N';                               /*  занулить PRNMET зн.'N'*/
               ...
               !strncmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "0H", 2)) {
      T_SYM[ITSYM].DLSYM = 2;
      T_SYM[ITSYM].PRPER = 'R';
      if (CHADR % 2) {
        CHADR = (CHADR / 2 + 1) * 2;
    ...
    if (!strncmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "0H", 2)) {
      if (CHADR % 2) {
        CHADR = (CHADR / 2 + 1) * 2;
      }
      CHADR += 2;
...
int SDS()                                         /*подпр.обр.пс.опер.DS    */
{
  ...
  if (TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND[0] == 'F') {
    ...
    RR.OP_RR.OP = 0;
    RR.OP_RR.R1R2 = 0;
    if (!strncmp(TEK_ISX_KARTA.STRUCT_BUFCARD.OPERAND, "BL2", 3)) {
      STXT(2);
    }
  } else {
    return 1;
