...
CONT2:
  ...
  endwin(); 
  END:
  printf ("\n%s\n", "zaverchenie obrabotki");

  return 0;
//Б Л О К  выдачи диагностических сообщений
ERR1:
  printf ("%s%s\n", "error while opening file with executable ", "modules");
  goto END;
...
ERR3:
  printf ("%s: %s\n" ,
   "error opening files" , SPISOK [I] );
  goto END;

ERR4:
  printf ("%s\n" ,
   "pepolnenie spiska sobiraemih modulei" );
  goto END;

ERR5:
  printf ("%s\n" ,
   "peperolnenie bufera objektnih kart");
  goto END;
...
ERR7:
  printf("Addressing error - interruption\n");
  goto END;
